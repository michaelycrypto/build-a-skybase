--[[
	FurnaceUI.lua
	Interactive smelting UI with temperature balance mini-game
	Redesigned to match the voxel inventory/worlds panel aesthetic.

	Features:
	- Recipe selection grid showing available smelting recipes
	- Temperature gauge mini-game (hold to heat, release to cool)
	- Countdown-driven progress (auto smelts, interaction speeds it up)
...
local function CreateStyledButton(config)
	...
	if style == "accent" then
		bgColor = FURNACE_LAYOUT.BTN_ACCENT
		hoverColor = FURNACE_LAYOUT.BTN_ACCENT_HOVER
		textColor = FURNACE_LAYOUT.TEXT_PRIMARY
	elseif style == "danger" then
		bgColor = Color3.fromRGB(220, 100, 100)
		hoverColor = Color3.fromRGB(240, 120, 120)
		textColor = FURNACE_LAYOUT.TEXT_PRIMARY
	else -- "default"
		bgColor = FURNACE_LAYOUT.BTN_DEFAULT
		hoverColor = FURNACE_LAYOUT.BTN_DEFAULT_HOVER
		textColor = FURNACE_LAYOUT.TEXT_PRIMARY
		bgTransparency = FURNACE_LAYOUT.SLOT_BG_TRANSPARENCY
	...
	corner.CornerRadius = UDim.new(0, FURNACE_LAYOUT.SLOT_CORNER_RADIUS)
	corner.Parent = button

	-- Border stroke
	local border = Instance.new("UIStroke")
	border.Name = "Border"
	border.Color = FURNACE_LAYOUT.SLOT_BORDER_COLOR
	border.Thickness = FURNACE_LAYOUT.SLOT_BORDER_THICKNESS
	...
	if useBackgroundImage then
		...
		bgCorner.CornerRadius = UDim.new(0, FURNACE_LAYOUT.SLOT_CORNER_RADIUS)
...
local function SetButtonDisabled(button, disabled, disabledText)
	...
	if disabled then
		button.BackgroundColor3 = FURNACE_LAYOUT.BTN_DISABLED
		button.BackgroundTransparency = 0
		button.TextColor3 = FURNACE_LAYOUT.BTN_DISABLED_TEXT
		if disabledText then
			button.Text = disabledText
		end
	else
		button.BackgroundColor3 = FURNACE_LAYOUT.BTN_ACCENT
		button.BackgroundTransparency = 0
		button.TextColor3 = FURNACE_LAYOUT.TEXT_PRIMARY
	end
end
...
function FurnaceUI.new(inventoryManager)
	...
	self.indicator = SmeltingConfig.Gauge.START_POSITION
	self.zoneCenter = 50
...
function FurnaceUI:Initialize()
	...
	if not eventsSuccess then
		warn("[FurnaceUI] Failed to register events:", eventsError)
	end
...
function FurnaceUI:CreatePanel()
	local totalWidth = FURNACE_LAYOUT.TOTAL_WIDTH
	local totalHeight = FURNACE_LAYOUT.HEADER_HEIGHT + FURNACE_LAYOUT.BODY_HEIGHT
...
function FurnaceUI:CreateContentColumn(parent)
	local columnWidth = FURNACE_LAYOUT.TOTAL_WIDTH
	local columnHeight = FURNACE_LAYOUT.BODY_HEIGHT
	local shadowHeight = FURNACE_LAYOUT.SHADOW_HEIGHT
	...
	local border = Instance.new("UIStroke")
	border.Name = "Border"
	border.Color = FURNACE_LAYOUT.COLUMN_BORDER_COLOR
	border.Thickness = FURNACE_LAYOUT.COLUMN_BORDER_THICKNESS
...
function FurnaceUI:CreateRecipeSection(parent)
	-- Section label
	local label = Instance.new("TextLabel")
	label.Name = "RecipeLabel"
	label.Size = UDim2.new(1, 0, 0, FURNACE_LAYOUT.LABEL_HEIGHT)
	label.BackgroundTransparency = 1
	label.Font = Enum.Font.Code
	label.TextSize = LABEL_SIZE
	label.TextColor3 = FURNACE_LAYOUT.TEXT_MUTED
...
function FurnaceUI:CreateRecipeInfo(parent)
	...
	recipeName.TextColor3 = FURNACE_LAYOUT.TEXT_PRIMARY
	...
	reqLabel.TextColor3 = FURNACE_LAYOUT.TEXT_MUTED
	reqLabel.Text = "NEEDS"
...
function FurnaceUI:CreateMiniGamePanel()
	...
	itemGlow.Color = FURNACE_LAYOUT.COLD_COLOR
	...
	heatBar.BackgroundColor3 = FURNACE_LAYOUT.SLOT_BG_COLOR
	...
	progressBar.BackgroundColor3 = FURNACE_LAYOUT.SLOT_BG_COLOR
	...
	statsLine.TextColor3 = FURNACE_LAYOUT.TEXT_PRIMARY
	statsLine.Text = "TIME: --"
	...
	hintLabel.TextColor3 = FURNACE_LAYOUT.TEXT_MUTED
...
function FurnaceUI:CreateCompletionPanel()
	...
	local completionLayout = {
		topPadding = 0,
		headerHeight = FURNACE_LAYOUT.HEADER_HEIGHT,
		headerToItem = 28,
		itemSize = 100,
		itemToLabel = 24,
		labelHeight = 28,
	...
	outputBorder.Color = FURNACE_LAYOUT.BTN_ACCENT
	...
	outputLabel.TextColor3 = FURNACE_LAYOUT.TEXT_PRIMARY
	...
	effLabel.TextColor3 = FURNACE_LAYOUT.ZONE_COLOR
	effLabel.Text = "GREAT"
...
function FurnaceUI:CreateRecipeCard(recipe, index)
	...
	local border = Instance.new("UIStroke")
	border.Name = "Border"
	border.Color = recipe.canCraft and FURNACE_LAYOUT.BTN_ACCENT or FURNACE_LAYOUT.SLOT_BORDER_COLOR
	border.Thickness = FURNACE_LAYOUT.SLOT_BORDER_THICKNESS
	...
	fuelText.TextColor3 = recipe.hasEnoughFuel and FURNACE_LAYOUT.BTN_ACCENT or FURNACE_LAYOUT.HOT_COLOR
	fuelText.ZIndex = 5
	...
	if not recipe.canCraft then
		...
		overlayCorner.CornerRadius = UDim.new(0, FURNACE_LAYOUT.SLOT_CORNER_RADIUS)
	...
	card.MouseEnter:Connect(function()
		if recipe.canCraft then
			...
			TweenService:Create(card, TweenInfo.new(0.1), {
				BackgroundColor3 = FURNACE_LAYOUT.SLOT_HOVER_COLOR
			}):Play()
		end
	end)
	...
	card.MouseLeave:Connect(function()
		if self.selectedRecipe ~= recipe then
			border.Transparency = recipe.canCraft and 0.6 or 0
			TweenService:Create(card, TweenInfo.new(0.1), {
				BackgroundColor3 = FURNACE_LAYOUT.SLOT_BG_COLOR
			}):Play()
		end
	end)
...
function FurnaceUI:SelectRecipe(recipe, card)
	-- Deselect previous
	if self.selectedRecipeCard then
		...
		if prevData then
			...
			prevData.border.Color = self.selectedRecipe.canCraft and FURNACE_LAYOUT.BTN_ACCENT or FURNACE_LAYOUT.SLOT_BORDER_COLOR
			prevData.card.BackgroundColor3 = FURNACE_LAYOUT.SLOT_BG_COLOR
		end
	end
	...
	if cardData then
		...
		cardData.border.Color = FURNACE_LAYOUT.BTN_ACCENT
		cardData.card.BackgroundColor3 = FURNACE_LAYOUT.SLOT_HOVER_COLOR
	end
...
function FurnaceUI:UpdateRecipeInfo(recipe)
	-- Update name
	...
	for i, ing in ipairs(recipe.ingredients) do
		...
		countLabel.TextColor3 = ing.owned >= ing.required and FURNACE_LAYOUT.ZONE_COLOR or Color3.fromRGB(255, 100, 100)
	...
	fuelLabel.TextColor3 = FURNACE_LAYOUT.TEXT_PRIMARY
...
function FurnaceUI:StartMiniGame(smeltConfig)
	if not smeltConfig then
		warn("[FurnaceUI] StartMiniGame: smeltConfig is nil!")
		return
	end
	...
	self.autoDuration = self.fastDuration * SmeltingConfig.Countdown.IDLE_MULTIPLIER
	self.timeRemaining = self.autoDuration
	self.isHeating = false

	-- Switch to immersive smelting mode (darker backdrop)
	UIVisibilityManager:SetMode("smelting")
	...
	if self.itemGlow then
		self.itemGlow.Color = FURNACE_LAYOUT.COLD_COLOR
	...
	if self.statsLine then
		...
		self.statsLine.TextColor3 = FURNACE_LAYOUT.TIME_IDLE_COLOR
	end
	...
	if self.miniGamePanel then
		...
		warn("[FurnaceUI] miniGamePanel is nil! Cannot show mini-game UI")
		return
	end
...
function FurnaceUI:UpdateMiniGame(dt)
	...
	if self.zoneCenter >= SmeltingConfig.ZoneDrift.MAX then
		self.zoneCenter = SmeltingConfig.ZoneDrift.MAX
		self.driftDirection = -1
	elseif self.zoneCenter <= SmeltingConfig.ZoneDrift.MIN then
		self.zoneCenter = SmeltingConfig.ZoneDrift.MIN
		self.driftDirection = 1
	end

	-- Update temperature indicator
	if self.isHeating then
		self.indicator = self.indicator + (SmeltingConfig.Gauge.HEAT_RATE * dt)
	else
		self.indicator = self.indicator - (SmeltingConfig.Gauge.COOL_RATE * dt)
	end
	self.indicator = math.clamp(self.indicator, SmeltingConfig.Gauge.MIN, SmeltingConfig.Gauge.MAX)

	-- Countdown progress: auto smelts, interaction speeds it up
	local countdownRate = self.isHeating and SmeltingConfig.Countdown.IDLE_MULTIPLIER or 1
...
function FurnaceUI:UpdateMiniGameUI()
	...
	local tempColor
	if t < 0.5 then
		tempColor = FURNACE_LAYOUT.COLD_COLOR:Lerp(Color3.fromRGB(255, 180, 80), t * 2)
	else
		tempColor = Color3.fromRGB(255, 180, 80):Lerp(FURNACE_LAYOUT.HOT_COLOR, (t - 0.5) * 2)
	end

	-- Item border: subtle temperature feedback
	if self.itemGlow then
		local targetColor = inZone and FURNACE_LAYOUT.ZONE_COLOR or tempColor
	...
	local targetIndicatorColor = inZone and FURNACE_LAYOUT.ZONE_COLOR or tempColor
	...
	if self.statsLine then
		...
		self.statsLine.TextColor3 = self.isHeating and FURNACE_LAYOUT.TIME_ACTIVE_COLOR or FURNACE_LAYOUT.TIME_IDLE_COLOR
	end
	...
	if self.hintLabel then
		if self.isHeating then
			...
			self.hintLabel.TextColor3 = FURNACE_LAYOUT.ZONE_COLOR
			self.hintLabel.TextTransparency = 0
		else
			self.hintLabel.Text = "AUTO - HOLD TO SPEED"
			self.hintLabel.TextColor3 = FURNACE_LAYOUT.TEXT_MUTED
...
function FurnaceUI:OnEnterZone()
...
function FurnaceUI:GetEfficiencyRating(percent)
	if percent >= 90 then
		return "PERFECT", FURNACE_LAYOUT.ZONE_COLOR
...
function FurnaceUI:CompleteMiniGame()
	-- Stop update loop
	...
	EventManager:SendToServer("RequestCompleteSmelt", {
		furnacePos = self.furnacePosition,
		efficiencyPercent = efficiencyPercent
	})
end

function FurnaceUI:OnCancelSmelt()
	...
	EventManager:SendToServer("RequestCancelSmelt", {
		furnacePos = self.furnacePosition
	})
...
function FurnaceUI:ShowCompletion(data)
	self.panel.Visible = false
...
function FurnaceUI:PlayParticleBurstEffect(parent, color, particleCount)
	particleCount = particleCount or 12
	color = color or FURNACE_LAYOUT.BTN_ACCENT
...
function FurnaceUI:PlayShimmerEffect(parent)
...
function FurnaceUI:PlayGlowPulseEffect(frame, color)
	color = color or FURNACE_LAYOUT.BTN_ACCENT
...
function FurnaceUI:PlayRevealAnimation()
	...
	task.delay(0.55, function()
		local autoBgTransparency = self.autoSmeltEnabled and 0 or FURNACE_LAYOUT.SLOT_BG_TRANSPARENCY
		...
		TweenService:Create(self.recipesBtn, FADE, {
			BackgroundTransparency = FURNACE_LAYOUT.SLOT_BG_TRANSPARENCY,
			TextTransparency = 0
		}):Play()
	end)
...
function FurnaceUI:StartCompletionGlowPulse()
...
function FurnaceUI:StopCompletionGlowPulse()
...
function FurnaceUI:CancelAutoSmeltTimer()
...
function FurnaceUI:UpdateAutoSmeltButton()
	...
	if enabled then
		self.autoSmeltBtn.BackgroundColor3 = FURNACE_LAYOUT.BTN_ACCENT
		self.autoSmeltBtn.BackgroundTransparency = 0
	else
		self.autoSmeltBtn.BackgroundColor3 = FURNACE_LAYOUT.BTN_DEFAULT
		self.autoSmeltBtn.BackgroundTransparency = FURNACE_LAYOUT.SLOT_BG_TRANSPARENCY
	end
	self.autoSmeltBtn.TextColor3 = FURNACE_LAYOUT.TEXT_PRIMARY
end

function FurnaceUI:SetAutoSmeltEnabled(enabled)
...
function FurnaceUI:StartAutoSmeltTimer()
	self:CancelAutoSmeltTimer()
...
function FurnaceUI:OnSmeltAgain()
	self:CancelAutoSmeltTimer()
	...
	EventManager:SendToServer("RequestOpenFurnace", self.furnacePosition)
end

-- Called after receiving updated recipes to check if we can smelt again
function FurnaceUI:TrySmeltAgain()
...
function FurnaceUI:ShowRecipeSelection()
	...
	if self.recipesBtn then
		self.recipesBtn.BackgroundTransparency = FURNACE_LAYOUT.SLOT_BG_TRANSPARENCY
...
function FurnaceUI:OnStartSmelt()
	self:CancelAutoSmeltTimer()
	...
	if not self.furnacePosition then
		return
	end
	...
	EventManager:SendToServer("RequestStartSmelt", {
...
function FurnaceUI:ShowSmeltError(message)
	...
	if self.startButton then
		...
		self.startButton.TextColor3 = FURNACE_LAYOUT.TEXT_PRIMARY
...
function FurnaceUI:RegisterEvents()
	-- Furnace opened
	EventManager:RegisterEvent("FurnaceOpened", function(data)
		self:Open(data)
	end)

	-- Smelt started
	EventManager:RegisterEvent("SmeltStarted", function(data)
		-- Reset button state
		self.isWaitingForSmelt = false
		if self.startButton then
			self.startButton.Text = "SMELT"
		...
		if type(data) == "table" then
			...
			if data.smeltConfig then
				...
				if not success then
					warn("[FurnaceUI] StartMiniGame error:", err)
	...
	-- Smelt completed
	EventManager:RegisterEvent("SmeltCompleted", function(data)
...
function FurnaceUI:Open(data)
	if not data then
		warn("[FurnaceUI] Open called with no data!")
		return
	end

	self.isOpen = true
	self.furnacePosition = {x = data.x, y = data.y, z = data.z}
...
function FurnaceUI:Toggle()
	if self.isOpen then
		self:Close()
	else
		-- Toggle requires furnace position, so we can't just open without data
		-- This is typically not used for FurnaceUI since it requires interaction
		warn("[FurnaceUI] Toggle called but FurnaceUI requires furnace position to open")
	end
end
...
function FurnaceUI:_getKeys(tbl)
	local keys = {}
