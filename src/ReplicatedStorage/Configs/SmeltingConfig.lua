--[[
	...
	Temperature gauge settings, difficulty scaling per ore tier,
	and efficiency calculations for the furnace system.
]]
...
SmeltingConfig.MAX_INTERACTION_DISTANCE = 18 -- 6 blocks * 3 studs
...
function SmeltingConfig:GetDifficulty(recipeId)
...
function SmeltingConfig:GetTier(recipeId)
...
function SmeltingConfig:GetProgressRate(indicator, zoneCenter, zoneWidth)
...
function SmeltingConfig:CalculateEfficiency(timeInZone, totalTime)
	if totalTime <= 0 then
...
function SmeltingConfig:CalculateCoalCost(baseCoal, efficiencyMultiplier)
	local adjustedCost = baseCoal * efficiencyMultiplier
	return math.max(1, math.ceil(adjustedCost))
end

--[[
	Check if a recipe requires furnace
	@param recipe: table - Recipe from RecipeConfig
	@return: boolean
]]
function SmeltingConfig:RequiresFurnace(recipe)
	return recipe and recipe.requiresFurnace == true
end
